...
class App {
  constructor() {
    ...
    const actV = new ActionsView(api, state, toast);
    actV._populateTimeZones();

    this._api = api; this._apiLogger = apiLogger; this._state = state; this._cfg = cfg;
    this._devV = devV; this._tagV = tagV; this._manV = manV; this._actV = actV;
  ...
  _bindNav() {
    ...
    $('#api-log-export').on('click', () => {
      const data = this._apiLogger.export();
      navigator.clipboard.writeText(data).then(() => this._cfg.setStatus(true, 'API log copied to clipboard'));
    });
  ...
  _renderApiLog(query = '') {
    ...
    const rows = logs.map(log => {
      ...
      return `<tr>
        ...
        <td class="${statusClass}">${log.status || 'ERR'}</td>
  ...
  navigateTo(view) {
    if (this._devV && view !== 'devices' && view !== 'home') {
      this._devV.clearCache();
    }
  ...
  async _init() {
    if (!this._api.apiKey) return;
    try {
      const [tags, mandates, profiles] = await Promise.all([
        this._api.fetchAllPages('/tags', { limit: 100, sort: 'name', sort_direction: 'ascending' }),
        this._api.fetchAllPages('/mandates', { limit: 100, sort: 'name', sort_direction: 'ascending' }),
        this._api.fetchAllPages('/profiles', { limit: 100, sort: 'name', sort_direction: 'ascending' })
      ]);
      this._state.setTags(tags);
      this._state.setMandates(mandates);
      this._state.setProfiles(profiles);
