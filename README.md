...
- **Scopes**: devices and gateways have separate scopes – mandates (multi-select), folders (multi-select, any of) and a name filter each. Scopes are applied with documented simple filters (`/tags/:id/devices` per folder, `mandate_id_is`, `name_ilike`) and re-checked client-side. With *Element LNS only* (default) devices without an ELEMENT LNS interface are skipped – by driver instance (`GET /drivers/instances`, tried once per server) or, if the key cannot list driver instances, by the LNS interface options.
- **Gateways** are devices with a gateway-management interface (`opts.gateway_id`); ELEMENT does not return a device `type` on every instance. AbacusSql filters on interface options are not used (some servers answer them with HTTP 500). Without a gateway scope all devices are scanned once (cached 15 min) – set the gateway scope to the gateway mandate/folder to make this cheap. Gateways seen in packets but outside the scope are looked up via the `by-eui` selector (max. 40 per run); gateways not visible to the key are shown as *external*. Packets are matched via `gateways[].gateway_id` (or `meta.gateway_stats[].router_id_hex`). Gateway state: *receiving*, *idle*, *offline* (last packet-forwarder ping older than 1 h), *external*.
- **Resilience**: transient server errors (e.g. database pool timeouts, HTTP 5xx) are retried with back-off; ELEMENT's `error` message is shown.
