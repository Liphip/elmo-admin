...
- **Scope**: with *Element LNS only* (default) devices without an interface on an ELEMENT LNS driver instance (`GET /drivers/instances`) are skipped, so wM-Bus, NB-IoT or virtual devices do not show up as "silent".
- **Gateways** are matched to Element devices of type `gateway` by the EUI in their interface options (or an EUI in their slug/name). Gateways that only appear in packet metadata are shown as *external*.
